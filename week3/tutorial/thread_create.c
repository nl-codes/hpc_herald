...
void *(*start_routine)(void *) --->	Pointer to the function that the new thread will execute. 👉 The function must take a void* argument and return a void*.
void *arg --->                  	Argument passed to the thread function. It can be NULL or a pointer to any data the thread needs.

⚙️ 3️⃣ Return Value
Returns 0 on success, or an positive error number on failure.
*/
